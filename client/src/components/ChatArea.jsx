...
import { Reply, Smile, X, Phone, Video, Search, PanelRightClose } from 'lucide-react';
...
const ChatArea = () => {
  ...
  /* ------------------------------------------------------------------
     Background theme — read from selectedChat (the Chat document)
     so it works for BOTH groups AND 1-on-1 conversations.
     ------------------------------------------------------------------ */
  ...
  const friend = selectedChat 
    ...
    : (selectedUser?.users ? selectedUser.users.find(u => u._id !== authUser._id) : selectedUser);
    
  const isOnline  = friend ? useAuthStore.getState().onlineUsers.includes(friend._id) : false;
    ...
    <div className={`${s.chatRoot} bg-white`} style={bgStyle}>
      ...
      <div className={s.header}>
        ...
        <div>
          ...
          {selectedUser?.isGroupChat ? (
            <p className={s.headerSubMembers}>{selectedUser.users?.length} members</p>
          ) : friend ? (
            isOnline
              ? <p className={s.headerSubOnline}>Online</p>
              : <p className={s.headerSubOffline}>
                  Offline — {friend.lastActive ? format(friend.lastActive) : 'Unknown'}
                </p>
          ) : null}
        </div>
              ...
              <button 
                ...
                onClick={() => {
                  if (isGroup) {
                    ...
                    startCall(friend._id, friend.username || friend.fullName, 'voice');
                  }
                }}
              ...
              <button 
                ...
                onClick={() => {
                  if (isGroup) {
                    ...
                    startCall(friend._id, friend.username || friend.fullName, 'video');
                  }
                }}
      ...
      <div className={s.messagesList}>
        {messages.map((message) => {
          ...
          return (
            <div key={message._id} className={isMe ? s.messageRowMe : s.messageRowOther}>
                ...
                {!isMe && (
                  <div className={s.senderAvatar}>
                    {message.senderId.profilePic
                      ? <img src={message.senderId.profilePic} alt="avatar" className="size-full object-cover" />
                    ...
                    {activeReactionId === message._id && (
                      <div className="absolute z-50 -top-10 right-0 bg-white dark:bg-slate-800 shadow-lg border border-gray-200 dark:border-slate-700 rounded-full px-2 py-1 flex gap-2">
                        {REACTION_EMOJIS.map(emoji => (
                          <button key={emoji} onClick={() => { reactToMessage(message._id, emoji); setActiveReactionId(null); }} className="hover:scale-125 transition-transform text-lg">{emoji}</button>
                        ))}
                      </div>
                    )}
                    <button onClick={() => setActiveReactionId(activeReactionId === message._id ? null : message._id)} className="p-1 text-gray-400 hover:text-brand-red"><Smile className="size-4" /></button>
                    <button onClick={() => setReplyingTo(message)} className="p-1 text-gray-400 hover:text-brand-red"><Reply className="size-4" /></button>
                    <button
                ...
                <div className="flex flex-col relative max-w-[70%]">
                  ...
                  {!isMe && selectedChat?.isGroupChat && (
                    <span className="text-xs text-gray-500 mb-1 ml-1 font-medium">
                      {selectedChat.nicknames?.[message.senderId._id] || message.senderId.username}
                    </span>
                  )}
                  ...
                  <div className={isMe ? s.bubbleMe : s.bubbleOther}>
                    ...
                    {message.replyTo && (
                      <div className="mb-2 p-2 bg-black/10 dark:bg-white/10 rounded border-l-4 border-brand-red text-xs opacity-90 cursor-pointer">
                        <p className="font-semibold mb-0.5">{message.replyTo.senderId?.username || 'User'}</p>
                        <p className="truncate line-clamp-1">{message.replyTo.messageType === 'text' ? message.replyTo.text : `[${message.replyTo.messageType}]`}</p>
                      </div>
                    )}

                    {message.messageType === 'image' && message.image && (
                      <img src={message.image} alt="Attachment" className={s.msgImage} />
                    )}
                    ...
                    {message.messageType === 'audio' && message.audio && (
                      <audio controls src={message.audio} className="max-w-[200px] sm:max-w-[250px] outline-none h-10 rounded-full" />
                    )}
                    {message.text && (
                  ...
                  {message.reactions && Object.keys(message.reactions).length > 0 && (
                    <div className={`absolute -bottom-3 ${isMe ? 'right-2' : 'left-2'} flex gap-1 bg-white dark:bg-slate-800 shadow-sm border border-gray-100 dark:border-slate-700 rounded-full px-1.5 py-0.5 text-[11px] z-10`}>
                      {Object.entries(message.reactions).map(([emoji, users]) => (
                        <span key={emoji} onClick={() => reactToMessage(message._id, emoji)} className="cursor-pointer hover:scale-110" title={users.length > 0 ? `${users.length} reactions` : ''}>
                          {emoji} {users.length > 1 && <span className="ml-0.5 text-gray-500">{users.length}</span>}
                        </span>
                      ))}
                    </div>
                  )}
