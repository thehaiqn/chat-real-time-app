...
export const updateProfile = async (req, res) => {
  try {
    ...
    // Kiểm tra xem email hoặc tên người dùng đã được người khác sử dụng hay chưa.
...
export const addFriend = async (req, res) => {
  try {
    ...
    // Tránh gửi mật khẩu về phía client
    friend.password = undefined;

    // Phát sự kiện cho cả hai người dùng
    ...
    res
      .status(200)
      .json({ message: "Friend added successfully", newFriend: friend });
  } catch (error) {
...
export const unfriendUser = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Unfriended successfully", friends: user.friends });
  } catch (error) {
...
export const blockUser = async (req, res) => {
  try {
    ...
    // Tìm cuộc trò chuyện 1-on-1 giữa hai người dùng này và xóa nó
    ...
    if (chat) {
      ...
      // Gửi sự kiện `chat-deleted` tới cả hai người dùng qua socket hoặc tới phòng chat.
    ...
    // Hiển thị danh sách người dùng bị chặn để trả về kết quả
